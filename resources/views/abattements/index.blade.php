...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800"><strong>{{ ('Liste des indemnités') }}</strong></h1>
</div>
      ...
      <div class="card-body">
        ...
        <div class="dt-responsive table-responsive">
            ...
            <tbody>
                 ...
                 <tr>
                    <td><span> {{ $indemnite->id }} </span></td>
                    <td><span> {{ $indemnite->libelle }} </span></td>
                    <td>
                        <a href="" data-toggle="modal" data-target="#exampleModal1"
                        id="l{{ $indemnite->id }}"
                        data-route="{{ route('modifindemnite', $indemnite->id) }}"
                        data-libelle="{{ $indemnite->libelle }}"
                        onclick="updateindemnite('#l{{ $indemnite->id }}')"
                        class="btn btn-warning">
                            <span class="fas fa-fw fa-edit"></span>
                        </a>
                   </td>
                    <td>
                        <form method="POST" action="{{ route('indemnites.destroy', $indemnite) }}" id="form{{ $indemnite->id }}">

                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}

                            <button type="button" onclick="confirmation('#form{{ $indemnite->id }}')" class="btn btn-danger" >
  ...
  <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <form method="POST" action="{{ route('indemnites.store') }}">
            @csrf
...
@section('js')
 ...
 <script src="{{ asset('vendor/datatables/dataTables.bootstrap4.min.js') }}"></script>
  ...
  <script>
    ...
    <script>
        function updateindemnite(indemnite){
            $('#libelle').val($(indemnite).attr('data-libelle'))
            $('#idft').attr('action',$(indemnite).attr('data-route'))
                }
    </script>
